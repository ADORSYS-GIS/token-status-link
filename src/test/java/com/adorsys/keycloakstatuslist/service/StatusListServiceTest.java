...
class StatusListServiceTest {
    ...
    @Test
    void isIssuerRegistered_SuccessScenarios() throws IOException, InterruptedException, StatusListException {
        // Create a new instance with performSleep override
        statusListService = new StatusListService(SERVER_URL, null, httpClient, RETRY_COUNT) {
            @Override
            protected void performSleep(long millis) throws InterruptedException {
                // Override to avoid actual sleep during tests
            }
        };

        // Test when issuer is registered (200 OK)
        setupMockResponse(200);
        assertTrue(statusListService.isIssuerRegistered(ISSUER_ID));
        verifyHttpClientCall(1);

        // Reset mocks for next test case
        reset(httpClient, httpResponse, httpHeaders);

        // Test when issuer is not registered (404 Not Found)
        setupMockResponse(404);
        assertFalse(statusListService.isIssuerRegistered(ISSUER_ID));
        verifyHttpClientCall(1);
    }

    @Test
    void isIssuerRegistered_ServerError() throws IOException, InterruptedException {
        // Create a new instance with performSleep override
        statusListService = new StatusListService(SERVER_URL, null, httpClient, RETRY_COUNT) {
            @Override
            protected void performSleep(long millis) throws InterruptedException {
        ...
        StatusListServerException exception = assertThrows(StatusListServerException.class,
                () -> statusListService.isIssuerRegistered(ISSUER_ID));
        assertEquals(500, exception.getStatusCode());
        verifyHttpClientCall(1);
    }

    @Test
    void isIssuerRegistered_MaxRetriesExceeded() throws IOException, InterruptedException {
        // Create a new instance with performSleep override
        statusListService = new StatusListService(SERVER_URL, null, httpClient, RETRY_COUNT) {
            @Override
            protected void performSleep(long millis) throws InterruptedException {
        ...
        StatusListException retryException = assertThrows(StatusListException.class,
                () -> statusListService.isIssuerRegistered(ISSUER_ID));
        
        String expectedMessage = "Failed to register issuer: " + ISSUER_ID + 
            " after " + RETRY_COUNT + " retries: Connection refused, Server URL: " + SERVER_URL;
        assertTrue(retryException.getMessage().contains(expectedMessage), 
            "Exception message should contain: " + expectedMessage + 
            " but was: " + retryException.getMessage());
            
        verify(httpClient, times(RETRY_COUNT + 1)).send(any(HttpRequest.class), any(HttpResponse.BodyHandler.class));
    }

    @Test
    void isIssuerRegistered_WithAuthToken() throws IOException, InterruptedException, StatusListException {
    ...
    // Helper method for setting up mock responses
    private void setupMockResponse(int statusCode) throws IOException, InterruptedException {
        when(httpResponse.statusCode()).thenReturn(statusCode);
        lenient().when(httpResponse.headers()).thenReturn(httpHeaders);
        lenient().when(httpHeaders.toString()).thenReturn("{}");
        when(httpClient.send(any(HttpRequest.class), any(HttpResponse.BodyHandler.class)))
                .thenReturn(httpResponse);
    }
